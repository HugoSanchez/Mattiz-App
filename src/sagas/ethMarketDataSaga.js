...
import {GET_ETH_MARKET_DATA} from '../actions/types'
...
import {getEthPrice, getHistoricPrices} from '../api/auth'
...
function* handlegasPriceAndFees(ethPrice) {
	...
	let formatedGasPrice = ethers.utils.formatEther(
		ethers.utils.bigNumberify(gasPrice * 1.5).toString(),
	)
	// Calculate simple transaction fee in dollars.
	let transactionFee = numeral(formatedGasPrice * ethPrice * 21000).format(
		'0,0.000',
	)

...
// Types.
import {GET_ETH_BALANCES} from '../actions/types'
...
function* handleEthBalances() {
	...
	// Get User from state.
	let user = yield select(userData)
	// Get Eth Price.
	let currentPriceETH = data.currentPriceETH
	// Start loop.
	while (true) {
		// Get Wallet Balance.
		let balance = yield Provider.getBalance(user.smartWalletAddress)
		// Parse Balance.
		let parsedBalance = ethers.utils.formatEther(
			ethers.utils.bigNumberify(balance).toString(),
		)
